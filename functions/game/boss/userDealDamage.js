...
module.exports = async function (session, boss, sendMessage) {
    try {
        let player;
        if (getPlayerClass(session)) {
            player = getPlayerClass(session);
        }
        ...
        if (player.stats.criticalDamage > 0) {
            criticalDamageInc += player.stats.criticalDamage;
        }
        ...
        if (player.stats.criticalChance > 0) {
