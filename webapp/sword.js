const REASONS = {
  cooldown: 'Сегодня попытка уже использована.',
};
...
export async function openSwordGame({ api, renderState, haptic, statusElement }) {
  ...
  overlay.innerHTML = `
    ...
    <div class="overlay-panel glass sword-panel">
      <header class="overlay-head">
        <div><div class="eyebrow">DAILY · SWORD</div><h2>Меч</h2></div>
  ...
  function resultBanner(payload) {
    ...
    banner.className = `sword-result ${growing ? 'positive' : delta < 0 ? 'negative' : 'neutral'}`;
    banner.innerHTML = `<span>${growing ? '⬆️' : delta < 0 ? '⬇️' : '➖'}</span><strong>${text}</strong>`;
    overlay.querySelector('.sword-panel').prepend(banner);
  ...
  async function roll() {
    ...
    feedback.textContent = 'Бросаем кубик на сервере…';
    haptic('heavy');

    try {
      const payload = await api('/api/sword/roll', { method: 'POST' });
      state = payload.sword;
      if (payload.state) renderState(payload.state);
      resultBanner(payload);
      feedback.textContent = payload.delta > 0
        ? `Меч вырос на ${payload.delta} мм.`
        : payload.delta < 0
          ? `Меч уменьшился на ${Math.abs(payload.delta)} мм.`
          : 'Длина меча не изменилась.';
      renderAll();
      ...
      if (error.payload?.sword) {
        state = error.payload.sword;
        renderAll();
      } else {
  ...
  function bind() {
    ...
    content.querySelector('[data-sword-refresh]')?.addEventListener('click', async () => {
      haptic('light');
      await refresh();
    });
  }
  ...
  function renderAll() {
    ...
    content.innerHTML = `
      <section class="sword-hero">
        <div class="sword-blade" aria-hidden="true"><span></span><i></i></div>
        <div class="sword-length"><small>ТЕКУЩАЯ ДЛИНА</small><strong>${formatNumber(state.length)} <em>мм</em></strong></div>
        <p>${modifierText(state)}</p>
      </section>

      <section class="sword-action-card">
        <div>
          <small>${canRoll ? 'ПОПЫТКА ГОТОВА' : 'СЛЕДУЮЩАЯ ПОПЫТКА'}</small>
          <strong data-sword-countdown>${canRoll ? 'Сейчас' : duration(state.remainMs)}</strong>
        </div>
        <button type="button" data-sword-refresh aria-label="Обновить">↻</button>
      </section>
      ...
      <div class="sword-rules">
        ...
        <span>⤵️ Негативный эффект из сундука также одноразовый.</span>
      </div>`;
    bind();
  }
  ...
  function tick() {
    ...
    if (node) node.textContent = remain > 0 ? duration(remain) : 'Сейчас';
    if (remain <= 0) {
      state = { ...state, canRoll: true, remainMs: 0 };
      renderAll();
    }
  }
