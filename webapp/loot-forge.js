...
export function renderForgeLootArt(item = {}, options = {}) {
  ...
  return `<div class="loot-forge-state forge-${forge.tier} condition-wear-${forge.wear}${hasImpact ? ` is-forge-impact forge-impact-${impactKind}` : ''}" data-forge-level="${forge.level}" data-forge-previous-level="${previous}" data-forge-tier="${forge.tier}" data-forge-segments="${forge.activeSegments}" data-forge-impact-kind="${impactKind}" style="--forge-progress:${forge.progress.toFixed(3)};--forge-level:${forge.level}"><span class="forge-aura-boost" aria-hidden="true"></span><span class="forge-rune-wheel" aria-hidden="true"></span>${renderLootArt(item, lootOptions)}<span class="forge-edge-light" aria-hidden="true"></span>${hasImpact ? forgeImpactMarkup(forge.level, impactKind) : ''}${levelMark}</div>`;
}
