...
module.exports = [[/(?:^|\s)\/football\b/, async (msg, session) => {
    ...
    if (session.game.football.isStart) {
        return sendMessageWithDelete(msg.chat.id, "Игра уже идёт. Команду нельзя вызвать повторно до окончания игры.", {}, 7000)
    }
    ...
    function startGame() {
        ...
        editMessageText('Бей. Ты выиграешь, если суммарное количество очков за 3 удара будет больше 12.', {
            message_id: id,
            chat_id: msg.chat.id,
            disable_notification: true,
            reply_markup: {
